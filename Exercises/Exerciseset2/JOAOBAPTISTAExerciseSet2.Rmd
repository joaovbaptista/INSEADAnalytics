...
*1. 6 components will explain 52.29% of the variance; i.e. you You need a minimum of 6 components 
<br>
*2. 5 yr T-Notes US (0.93)
<br>
*3.The first 10 components explain 62.61% of the variance and can be called principal components: 
<br>*Factor 1 Government bonds
<br>*Factor 2 FX 
<br>*Factor 3 EU - stock indices 
<br>*Factor 4 North-Amercican - stock indices
<br>*Factor 5 Euro-bonds and IR 
<br>*Factor 6 Oil
<br>*Factor 7 Metals
<br>*Factor 8 Foods
<br>*Factor 9 Gold & Sliver
<br>*Factor 10 Asian Stock Indices
<br>*4. See comments on charts below.
<br>*5.In the bull market more components influence the market as there are more occurences before the elbow of the chart. You can see for example that component 1 is less significant compared with the average over all the years. *In the bear market a) with the  first 4 factors we can explain  50% of the variance b) The correlation between futures increased.
*
...
*1. The code is varied by "MAfreq" which sequences in intervals of 20 from 10 to 250. <br>
The weight of each signal is calculated, by scaling the sum across all the futures, so that the total sum is 1. Then it uses the signal previously calculated to generate a following strategy using the weights we calculated 2 days ago to the futures.
<br>
*2. The command line MAfreq<-seq(10,250,by=20), alters how often the trading strategy is implemented.By changing it we achieve the follwoing portfolio return Ret:5.7, Vol:7.7, Sharpe:0.74, DD: 21.9 
*<br>
*
